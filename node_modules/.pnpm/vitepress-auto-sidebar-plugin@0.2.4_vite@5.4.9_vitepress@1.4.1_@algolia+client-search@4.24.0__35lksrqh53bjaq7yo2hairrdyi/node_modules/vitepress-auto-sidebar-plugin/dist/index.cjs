...
const log = consola.consola;

function getArticleData(path) {
  const file = node_fs.readFileSync(path, "utf-8");
  const { content, data } = matter__default(file);
...
function autoSidebarPlugin(options = {}) {
  ...
  return {
    ...
    config: async (config) => {
      ...
      const paths = (await glob__default(pattern, {
        cwd,
        ignore: [
          "**/node_modules/**",
          "**/dist/**",
    ...
    configureServer({
      ...
      watcher.add("*.md").on("all", perfectDebounce.debounce(async (type, path) => {
        if (type === "change") {
          const url = pathe.resolve(cwd) + pathe.sep;
...
function setItem(cwd, list, options, cache = {}, link = "") {
  if (!list.length)
    return void 0;
  let name = list.shift();
  link = pathe.join(link, name);
  const isFile = !list.length && Boolean(pathe.extname(name));
  let text = name;
  let index = getPathIndex(link);
  let fileOptions = {};
  if (isFile) {
    name = name.replace(pathe.extname(name), "");
    fileOptions = getArticleData(pathe.resolve(cwd, link));
...
function setDataFormat(cwd, paths, {
  ...
  list.forEach((path) => {
    const list2 = path.split(pathe.sep);
...
function generateSidebar(list) {
  const root = list.reduce((acc, { text, link, children, collapsed, groupAlone }) => {
    ...
    const key = `/${groupAlone ? link : link.split(pathe.sep)[0] || link}/`;
    if (!acc[key])
      acc[key] = [];
    acc[key].push(obj);
    return acc;
...
exports.default = autoSidebarPlugin;
exports.generateSidebar = generateSidebar;
exports.setDataFormat = setDataFormat;
