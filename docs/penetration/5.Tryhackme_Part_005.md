...
- 缺点： 需要`sudo` 权限，不稳定的应用，有可能会因SYN扫描导致关闭。
  ![](5.Tryhackme_Part_005-2.png)
  
![](5.Tryhackme_Part_005-3.png)
