...
function collectAttachmentsCurrentFolder(plugin, checking) {
  ...
  if (!checking) {
    (0, import_Queue.addToQueue)(plugin.app, () => collectAttachmentsInFolder(plugin, note?.parent ?? (0, import_Error2.throwExpression)(new Error("Parent folder not found"))));
  }
  return true;
}
...
var CustomAttachmentLocationPluginSettingsTab = class extends import_PluginSettingsTabBase.PluginSettingsTabBase {
  display() {
    ...
    new import_obsidian3.Setting(this.containerEl).setName("用连字符替换空格").setDesc("自动将附件文件夹和文件名中的空格替换为连字符。").addToggle((toggle) => (0, import_ValueComponent.extend)(toggle).bind(this.plugin, "replaceWhitespace"));
    new import_obsidian3.Setting(this.containerEl).setName("全部小写名称").setDesc("自动将文件夹名称和粘贴图像名称中的所有字符设置为小写。").addToggle((toggle) => (0, import_ValueComponent.extend)(toggle).bind(this.plugin, "toLowerCase"));
    new import_obsidian3.Setting(this.containerEl).setName("将粘贴的图像转换为 JPEG").setDesc("从剪贴板粘贴图像并将其转换为 JPEG。").addToggle((toggle) => (0, import_ValueComponent.extend)(toggle).bind(this.plugin, "convertImagesToJpeg"));
    ...
    new import_obsidian3.Setting(this.containerEl).setName("保留空附件文件夹").setDesc("如果启用，将保留空附件文件夹，这对于源代码控制很有用。").addToggle((toggle) => (0, import_ValueComponent.extend)(toggle).bind(this.plugin, "keepEmptyAttachmentFolders"));
    new import_obsidian3.Setting(this.containerEl).setName("删除孤立附件").setDesc("如果启用，当笔记被删除时，其孤立附件也将被删除。").addToggle((toggle) => (0, import_ValueComponent.extend)(toggle).bind(this.plugin, "deleteOrphanAttachments"));
  }
};
